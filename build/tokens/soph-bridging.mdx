...
- The adapters main delegate are also the same Sophon Foundation Multisig, and the delegate has permission to add new routes for new chains.
...
The foundation can just include new routes for new chains, and such changes require approval from LayerZero.

An script that shows and validates the configuration (and the approval needed) can be found [here](https://github.com/sophon-org/sophon-oft/blob/main/layerzero.config.ts).