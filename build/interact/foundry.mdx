...
<Tip>
  In order to obtain a Sophscan API Key, you need to create an account and get
  an API Key
...
<Note>
  The paymaster allows you to perform gasless transactions, so there's no need
  to have a SOPH balance
</Note>
...
<Tip>
  To verify or use the paymaster, you can use the flags `--verify` and/or
  `----zk-paymaster-address` as explained above.
</Tip>
