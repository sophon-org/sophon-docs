...
<Info>
  **Overview**
  ...
  - To receive your **full 20% allocation**, delegate to both **Full Nodes** (automatic) _and_ **Light Nodes** (manual)
...
<Warning>
  **Memberships** that are **not active** for a given week will **_not_** receive the weekly reward.
</Warning>
...
Rewards are calculated and distributed on a weekly basis, **every Wednesday at 00:00 UTC**. After calculation, our systems begin creating the reward stream for each destination, represented by [vSOPH (Vesting SOPH)](https://explorer.sophon.xyz/address/0xe1a376fa16f40a1dc3712463616054e6cca338ed), an ERC20 like token that is **non-transferable**. Bear in mind this process can take some time – it’s normal for rewards to appear later in the day on Wednesday. Each reward stream has the following properties:
