...
- The rate (`pointsPerBlock`) is set by the **contract owner**.
- Points are distributed **proportionally** across all active pools.

### Pool Value Calculations

Each pool's value is calculated using the following formula **(note that the multiplier is `1e18` based):**
...
```
struct UserInfo {
    uint256 amount;        // Total stake (deposit + boost)
    uint256 boostAmount;   // Amount of boost applied
    uint256 depositAmount; // Original deposit
    uint256 rewardDebt;    // For reward calculations
    uint256 rewardSettled; // Accumulated points
}
```
...
<Info>
**Key Points**
1. Points generation is **block-based**.
2. Distribution is **proportional** to **pool value**.
3. Users earn based on their **stake size**.
4. **Boosts** increase effective stake.
5. Points are tracked **per share**.
</Info>
