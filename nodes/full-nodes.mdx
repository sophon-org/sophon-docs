...
<Tip>At present, anyone can run a **read-only** version of the Full Node.</Tip>
...
<Tip>
  For local testing, it's recommended to set up an in-memory node and fork
  mainnet using the [zkSync CLI](../build/interact/zksync-cli).
</Tip>

For detailed information on how to run, configure, and maintain a Full Node, please refer to:

- [zkSync Era External Node Documentation](https://docs.zksync.io/zksync-era/tooling/external-node)
- [ZKsync Core Documentation](https://matter-labs.github.io/zksync-era/core/latest/guides/external-node/01_intro.html)
